...
class FakeStatement{
  ...
  async first(){if(this.sql.startsWith("SELECT"))return{new_opportunities:2,new_sources:3,new_papers:5,source_changes:1,source_failures:0,source_runs:4,academic_runs:4};throw new Error(`Unexpected first: ${this.sql}`)}
  async all(){if(this.sql.includes("FROM opportunities"))return{results:[{name:"机会 A"}]};if(this.sql.includes("FROM sources"))return{results:[{name:"来源 A"}]};if(this.sql.includes("FROM papers"))return{results:[{name:"论文 A"}]};throw new Error(`Unexpected all: ${this.sql}`)}
...
test("refreshes daily, Monday-based weekly and monthly reports",async()=>{
  ...
  assert.deepEqual(reports[0].highlights,{opportunities:["机会 A"],sources:["来源 A"],papers:["论文 A"]});
