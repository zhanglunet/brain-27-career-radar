...
type Report={id:string;periodType:PeriodType;periodStart:string;periodEnd:string;newOpportunities:number;newSources:number;newPapers:number;sourceChanges:number;sourceFailures:number;sourceRuns:number;academicRuns:number;summary:string;highlights:{opportunities:string[];sources:string[];papers:string[]};generatedAt:string};
...
function ReportCard({report}:{report:Report}){return<article className={styles.reportCard}><header><div><span>{labels[report.periodType]}</span><h3>{periodLabel(report)}</h3></div><small>{report.sourceRuns} 次来源巡检 · {report.academicRuns} 次论文同步</small></header><div className={styles.reportNumbers}><div><b>{report.newOpportunities}</b><span>新机会</span></div><div><b>{report.newSources}</b><span>新来源</span></div><div><b>{report.newPapers}</b><span>新论文</span></div><div><b>{report.sourceChanges}</b><span>来源变化</span></div></div><p>{report.summary}</p><div className={styles.reportHighlights}>{highlight("机会",report.highlights.opportunities)}{highlight("来源",report.highlights.sources)}{highlight("论文",report.highlights.papers)}</div>{report.sourceFailures>0&&<p className={styles.reportWarning}>本周期有 {report.sourceFailures} 次来源检查失败；历史可信内容仍保留。</p>}</article>}
