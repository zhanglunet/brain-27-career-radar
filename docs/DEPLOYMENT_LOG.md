...
## 2026-08-02 · v0.13.0 心理学、脑科学与 AI 顶会雷达

- 发布前 `npm run verify` 全部通过：类型检查、ESLint、生产构建和 34 项测试成功；全新临时 D1 从 `0000` 至 `0023` 顺序迁移成功。
- 全新库验收得到 13 个会议、20 个关键节点，其中 18 个为精确日期、2 个仅确认月份；后两项未进入硬 Deadline。
- 生产迁移前导出 D1 SQL 备份，文件 1,930,905 bytes，SHA-256 为 `f6808ac9ef3e6f97f0276a3d05011233002a2bcf71b585fbb27b49275378c7d0`；备份未加入 Git。
- 生产迁移 `0023_academic_conference_radar.sql` 成功，新增会议、关键日期、官网版本和同步运行 4 张表；未删除或覆盖既有业务数据。
- 首轮受控 Cron 对 13 个会议官网执行巡检：12 个官网成功留存内容版本；COSYNE 官网出现 DNS/超时，保留已人工核验种子并由后续 Cron 重试，不影响会议与日历展示。
- 生产 API 验收：13 个会议、18 个精确节点、12 个已定日期会议、13 个可行动会议 Deadline；EACL 2027 的 2026-08-03 ARR 截止已进入日历；NeurIPS 全局搜索返回会议实体。
- `openagent.hk`、`www.openagent.hk`、`radar.openagent.hk` 的 `/conferences` 均返回 HTTP 200；会议 API、日历 API、搜索 API、系统状态 API 均返回 HTTP 200。

## 2026-08-02 · v0.11.1 全站手机导航

- 从提交 `7257539` 的已验证功能分支部署；发布前导出生产 D1 SQL 回滚基线：1,743,073 bytes，SHA-256 `4642d14039db56a98e1be000a74c53c0e51af62f06b248feee92e68fe6cbc52a`，仅保存在部署机临时目录，未提交 Git。
- 本次无 D1 schema 与采集管线变更；根布局新增统一手机导航，980px 以下替代各页面密集链接，提供四组主要入口、当前页高亮、背景滚动锁定、ESC/遮罩关闭和全局搜索入口。
- Worker 版本 `ed0733b1-c605-4b68-b46b-6e07b3c57960`；D1、Workers AI、Assets、Images 与 `0 1,7,13,19 * * *` Cron 绑定保持不变。
- `openagent.hk`、`www.openagent.hk`、`radar.openagent.hk` 的首页和 PRD 页共 6 个端点均为 HTTP 200，HTML 均包含手机导航触发器；客户端产物包含完整分组菜单与搜索入口，生产 CSS 包含 980px 显示规则。
- `www` 在发布传播窗口首次命中旧 HTML，随后使用唯一查询标识连续三次复查均加载新导航；完整质量门通过 TypeScript、ESLint、生产构建和 30/30 自动化测试。

## 2026-08-02 · v0.11.0 全局搜索与全部机会清单

- 从提交 `4ee1734` 的已验证功能分支部署；发布前导出生产 D1 SQL 回滚基线：1,743,073 bytes，SHA-256 `4642d14039db56a98e1be000a74c53c0e51af62f06b248feee92e68fe6cbc52a`，仅保存在部署机临时目录，未提交 Git。
- 本次无 D1 schema 变更；新增 `/search`、`/api/search`、`/opportunities` 和 `/api/opportunities`，继续只读现有事实表，并保持公开记录与候选记录隔离。
- Worker 版本 `d98e2e33-b8c0-4013-b910-4b5f85f97072`；D1、Workers AI、Assets、Images 与 `0 1,7,13,19 * * *` Cron 绑定保持不变。
- 生产全局检索“脑机接口”返回 31 条结果：机会 8、机构 8、导师 7、信息源 8；英国机会筛选返回 20 条。全部机会 API 返回 63 个公开机会，其中立即行动 25、硕士可申请 40、已有标准日期 8。
- `openagent.hk`、`www.openagent.hk`、`radar.openagent.hk` 的搜索页和全部机会页均为 HTTP 200；部署切换窗口出现过短暂旧版 404，随后加查询标识连续复查恢复为 200。两个新 API 也在 `radar.openagent.hk` 返回 HTTP 200。
- 完整质量门通过：TypeScript、ESLint、生产构建和 30/30 自动化测试；本地 D1 对六类联合搜索、英国筛选、网页 SSR 与 API 均完成冒烟验证。

## 2026-08-02 · v0.10.0 持续发现新机会、新公司和研究机构

- 从已提交功能分支部署；发布前导出生产 D1 SQL 回滚基线：1,187,635 bytes，SHA-256 `e970b022a1707cd9d3fd863435ea06c3c4152c0dcdf01b356c7bc70633809862`，仅保存在部署机临时目录，未提交 Git。
- 远程迁移 `0020_continuous_discovery.sql` 与 `0021_filter_organization_candidates.sql` 成功且无待执行项；新增可信目录、机构候选、独立运行记录及首轮噪声清理。
- Worker 最终版本 `2c683548-d393-4f92-97e2-0b51bc672b2e`；D1、Workers AI、Assets、Images 与 `0 1,7,13,19 * * *` Cron 保持绑定。
- 43 个已启用招聘列表进入通用同站新岗位发现；当前有 66 个未公开机会候选，全部保留证据且未自动写入公开机会库。
- 首次机构发现运行 5/5 目录成功、0 失败，原始检查发现 186 条符合机构词形的链接；规范化 URL 去重后当前有 206 个候选，其中中国 180、英国 26。抽样确认含中科院院属机构、MRC Centre for Neurodevelopmental Disorders、UK Dementia Research Institute 等；候选不等于方向相关或正在招聘。
- 首次验证发现 UKRI 社交/内容链接与中科院出版导航噪声后，先收紧机构名称词形和平台排除，再重跑目录；两次受控验证中的学术同步因仅验证机构管线而主动停止，相关 `academic_sync_runs` 已如实标记 `failed` 并写明原因，没有伪装成成功。
- `openagent.hk`、`www.openagent.hk`、`radar.openagent.hk` 的 `/discovery`、发现 API、`/campus-2027` 与 `/prd` 共 12 个端点全部 HTTP 200；发现 API返回 5 个目录、206 个机构候选、66 个机会候选和最近成功运行。
- 完整质量门通过：TypeScript、ESLint、生产构建、29/29 自动化测试，以及全新空 D1 从 `0000` 到 `0020` 顺序迁移；生产追加 `0021` 清理迁移后确认无待执行项。

## 2026-08-02 · v0.9.0 2027 校招与中英研究岗位专项

- 从提交 `dc897d7` 的干净功能分支部署；发布前导出生产 D1 SQL 回滚基线：1,147,558 bytes，SHA-256 `3187e73a1296ea983a829896efb9b1a1507518485353979b89f04c85d2b000c1`，仅保存在部署机临时目录，未提交 Git。
- 远程迁移 `0019_2027_campus_expansion.sql` 成功且无待执行项；生产 D1 为 116 个来源、63 个公开机会、58 个机构/公司，其中 22 项进入 2027 专项。
- 专项统计为：7 项立即行动、13 项持续关注、2 项等待开放；7 项已由官方明确面向 2027 届。中国相关 14 项、英国相关 10 项，跨地区机会可同时计入。
- 新增 `/campus-2027` 与 `/api/campus-2027`，支持国外大厂、国内大厂、大模型厂商、国内研究机构、英国研究机构、国外研究机构在中国六类赛道，以及地区、类型、状态和关键词筛选。
- Worker 版本 `acaf9685-13ba-4714-baa1-bdc1a9ef44fd`；D1、Workers AI、Assets、Images 与 `0 1,7,13,19 * * *` Cron 绑定保持不变。
- `openagent.hk`、`www.openagent.hk`、`radar.openagent.hk` 的专项页面、专项 API、首页和雷达 API 共 12 个端点全部 HTTP 200。裸域名在发布切换窗口短暂命中旧 404，随后连续三轮复查均为 200。
- 完整质量门通过：TypeScript、ESLint、生产构建、26/26 自动化测试，以及全新空 D1 从 `0000` 到 `0019` 顺序迁移。

## 2026-08-01 · v0.8.0 日报、周报与月报

- 从提交 `445c818` 的干净功能分支部署；发布前导出生产 D1 SQL 回滚基线：916,212 bytes，SHA-256 `737b8b06c48a909a1f0e4253ebd5a817fc6158bdc5cf12d4c0c604d151045a05`，仅保存在部署机临时目录，未提交 Git。
- 远程迁移 `0018_intelligence_reports.sql` 成功：新增日报、周报、月报事实快照，并按现有机会、来源、论文与同步日志的 UTC 日期回填历史数据；未修改或删除原始记录。
- 生产已生成 2 篇日报、1 篇周报和 2 篇月报。当前日报（2026-08-01 UTC）记录新增机会 31 个、信息源 94 个、论文 56 篇；2026-07-31 历史日报记录新增机会 14 个、信息源 15 个、论文 0 篇。
- Worker 版本 `8912ec63-d219-4e8f-9804-934b43dc2e5d`；原有 D1、Workers AI、Assets、Images 与 `0 1,7,13,19 * * *` Cron 绑定保持不变。每轮 Cron 在机会与论文流水线结束后刷新当天、当周和当月报告。
- `openagent.hk`、`www.openagent.hk`、`radar.openagent.hk` 的 `/reports` 与 `/api/reports?periodType=daily` 均返回 HTTP 200；裸域名在部署切换窗口曾短暂命中旧 404，随后复查恢复为 200。
- 完整质量门通过：TypeScript、ESLint、生产构建、26/26 自动化测试，以及全新空 D1 从 `0000` 到 `0018` 顺序迁移。

## 2026-08-01 · v0.7.0 大模型公司雷达与地图聚合下钻

- 从已提交的干净分支部署；发布前导出生产 D1 SQL 回滚基线：658,639 bytes，SHA-256 `000483914ea53faa0af1b6bf4b7b5f71bea828d465dcca23986cabcaf6f0edef`，仅保存在部署机临时目录，未提交 Git。
- 远程迁移 `0017_foundation_model_companies.sql` 成功且无待执行项；生产 D1 更新为 109 个来源、53 个机构/公司、45 个公开机会，其中 18 家大模型公司。
- 新增 18 个官方人才来源和 3 个已核验项目入口；字节跳动 Seed / 豆包“前沿技术领域人才校招”已关联官方页面。新增来源等待下一次自然 Cron 首次检查，页面明确显示该状态。
- Worker 版本 `0d74bcca-0932-41e3-9768-11e3ef45a071`；D1、Workers AI、Assets、Images 与 `0 1,7,13,19 * * *` Cron 绑定保持不变。
- `openagent.hk`、`www.openagent.hk`、`radar.openagent.hk` 的 `/ai-companies`、公司 API、`/map`、地图 API 和 `/prd` 共 15 个公开端点全部 HTTP 200。
- 公司 API 返回 18 家公司、18 个关联官方源、3 个已核验机会；地图 API 返回 45 个唯一机会，并按英国 12、中国内地 31、中国香港 2 聚合。
- 完整质量门通过：TypeScript、ESLint、生产构建、25/25 自动化测试，以及全新空 D1 从 `0000` 到 `0017` 顺序迁移。

## 2026-08-01 · v0.6.0 双语论文、知识图谱、日历、全球地图与区域页

- 发布前两次导出生产 D1 回滚基线：首次 553,718 bytes、SHA-256 `36bd9841b88d931d15f0d986f6574d788bbc7e4bb919287643be70131705144c`；区域扩展前 626,168 bytes、SHA-256 `e11671e3ae8c620f9eda656224eb7779c7358b7d30aad3daa095370cafceb602`。备份仅位于部署机临时目录，未提交 Git。
- 远程迁移 `0014`—`0016` 全部完成且无待执行项：新增双语字段、日期/时区/可信状态、北京与五个区域机构及官方来源。
- 最终 Worker 版本 `1f662116-4e36-4fe7-9ee8-d6e07b435c0e`，绑定 D1、Workers AI、Assets、Images 和 `0 1,7,13,19 * * *` Cron；三个自定义域名保持有效。
- 线上 `/papers`、`/knowledge-graph`、`/calendar`、`/map`、`/beijing`、`/shanghai`、`/shenzhen`、`/uk`、`/ireland`、`/hong-kong` 及对应 API 均返回 HTTP 200。
- 生产 D1 为 91 个来源、35 个机构、42 个公开机会、21 位导师、54 篇论文；全球地图按唯一机会统计为英国 10、中国内地 30、中国香港 2。
- 区域页机构/机会计数：上海 3/6、深圳 3/6、英国 7/10、爱尔兰 2/0、中国香港 4/2。爱尔兰无当前公开机会时明确显示等待发现。
- 历史论文通过仅本机可访问的临时 Worker 分批回填，最终 54/54 翻译完成、0 失败；52 篇有中英文摘要，2 篇来源无摘要。临时入口完成后删除，正式 Cron 每轮最多翻译 6 篇。
- 完整质量门通过：TypeScript、ESLint、生产构建、25/25 自动化测试，以及空 D1 从 `0000` 到 `0016` 顺序迁移。
